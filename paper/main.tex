...
\begin{quote}
\textit{Drift risk looks high for this session.}

\textit{Estimated drift risk: 76\%.}

\textit{Possible reasons: longer than intended, high scrolling.}

\textit{Still here for your original reason?}
\end{quote}

Example responses include \textit{Yes, continue}, \textit{I drifted, wrap up}, \textit{Remind me in 5 minutes}, and \textit{Save for later}. The displayed probability is phrased as an estimate rather than a fact. Reason cues are derived only from allowed metadata such as duration, scrolling, idle time, tab switching, or video playback status; they do not use page text, titles, comments, messages, recommendations, or search queries. If pilot participants find exact percentages confusing or stressful, the study interface can show only a coarse risk label such as \textit{low}, \textit{medium}, or \textit{high}, while still exporting the numeric probability for analysis.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \texttt{model\_risk\_level}, for the model-assisted condition
  \item \texttt{model\_reason\_cues}, for the model-assisted condition
  ...
  \item \texttt{model\_probability\_shown\_to\_user}, for the model-assisted condition
...
\begin{figure}[t]
  ...
  \Description{A flow diagram showing a monitored domain opening, an optional intention prompt with seven neutral intended-activity choices, browsing with activity windows, a post-session reflection prompt, and the mapping from reflection answers to non-drift, drift, or separate non-binary outcomes.}
  \label{fig:labeling-flow}
\end{figure}

\subsection{Dashboard and Export}
The participant-facing dashboard is a local reflection and transparency interface organized into six views. An \textit{Overview} reports monitored and labeled sessions, self-reported drift rate, average duration, monitoring status, and seven-day and study-to-date summaries. \textit{Trends} visualizes daily and weekly session counts, duration, and drift rate, with optional time-of-day and day-of-week breakdowns. \textit{Intention Alignment} compares intention categories, intended and actual duration where available, domains, and post-session outcomes. These views use descriptive language and do not infer productivity, attention, addiction, or mental state.

The \textit{Model Transparency} view summarizes how many model-assisted check-ins were shown, their estimated probabilities and risk levels, the privacy-safe reason cues displayed, participant responses, and ratings of timing, usefulness, or annoyance. Model estimates are visually separated from self-reported outcomes. The interface states that risk is an estimate used to time reflection, not proof that drift occurred. Exact probabilities can be replaced by low, medium, and high labels if pilot feedback indicates that percentages are confusing or stressful.

The \textit{Session History} view provides a filterable table by date, monitored domain, condition, declared intention, reflection label, and model risk level. A session-detail view shows duration, aggregate activity counts, post-session reflection, and check-in outcome, but never page titles, full URLs, page text, screenshots, messages, or keystroke values. The \textit{Data and Privacy} view allows participants to pause monitoring, configure domains, inspect field definitions, delete an individual session, delete all local data, and export:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Participants can inspect delayed aggregate summaries and their own session records through the condition-neutral local dashboard.
  \item At the end of the study, participants export the dataset and complete a post-study survey; optional short interviews collect qualitative feedback about usefulness, burden, dashboard comprehension, and privacy concerns.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \item expected check-in rate at the selected threshold
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item session count by condition
  \item mean and median session duration by condition
  \item model check-ins shown
  \item percentage of sessions where a model check-in was shown
  \item average model drift probability when a check-in was shown
  \item displayed model risk level
  \item displayed model reason cues
  ...
  \item participant feedback on whether model-assisted check-ins appeared at appropriate times
  \item dashboard comprehension, perceived usefulness, and trust in the distinction between self-reported labels and model estimates
\end{itemize}
