...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Phase-consistent measurement.} The same intention and post-session questions should be used throughout collection and intervention so that model-assisted prompting is the only randomized difference.
  ...
  \item \textbf{Model-comparable outputs.} The exported schema should support weak baselines, tabular models, prediction at one, three, and five minutes, and condition-level drift-rate comparison.
  \item \textbf{Inspectable feedback.} Participants should be able to review aggregate trends, self-reported labels, and model-assisted events without exposing prohibited content or presenting model estimates as facts.
\end{enumerate}
...
\begin{figure}[t]
  ...
  \fbox{\parbox{0.94\linewidth}{\centering
    \textbf{Open monitored domain} $\rightarrow$ \textbf{Declare intention} $\rightarrow$ \textbf{Privacy-safe activity windows} $\rightarrow$ \textbf{Three-minute risk estimate}\\[4pt]
    Elevated risk $\rightarrow$ \textbf{1:1 randomization} $\rightarrow$ reflective prompt \textit{or} silent control\\[4pt]
    All sessions $\rightarrow$ \textbf{same post-session reflection} $\rightarrow$ \textbf{local storage and participant-controlled export}
  }}
...
\begin{figure}[t]
  \centering
  \IfFileExists{figures/figure2-v2.png}{%
    \includegraphics[width=\linewidth]{figures/figure2-v2.png}%
  }{%
    \fbox{\parbox{0.94\linewidth}{\centering
      \textbf{Open monitored domain} $\rightarrow$ \textbf{Declare intended activity} $\rightarrow$ \textbf{Browse session} $\rightarrow$ \textbf{Post-session reflection}\\[4pt]
      \small Work/study; learning/tutorial; specific information; communication/community; planned entertainment/break; open-ended browsing; accidental open\\[4pt]
      \textit{Yes, it matched} $\rightarrow 0$ \quad
      \textit{No, I drifted} $\rightarrow 1$ \quad
      Other responses $\rightarrow$ non-binary
    }}%
  }
...
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.94\linewidth}{\centering
    \textbf{Private participant exports} $\rightarrow$ schema and overlap checks $\rightarrow$ time-truncated features\\[4pt]
    $\rightarrow$ time/domain baselines; intention-only; activity-only; intention+activity logistic regression; optional Random Forest\\[4pt]
    $\rightarrow$ participant-aware and chronological evaluation $\rightarrow$ frozen three-minute model and threshold
  }}
  \caption{Planned modeling pipeline. A deliberately small set of tabular models is evaluated before one three-minute model is frozen for local intervention use.}
  \Description{A pipeline from private participant exports through validation, time-truncated features, baseline and logistic-regression models, participant-aware and chronological evaluation, and a frozen three-minute intervention model.}
