...
% Show complete author lists in every citation.
\let\shortcitep\citep
\renewcommand{\citep}[1]{\shortcitep*{#1}}
\setcitestyle{round}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\headingfont\fontsize{8.5}{10}\selectfont DRIFTSENSE}
\fancyhead[R]{\headingfont\fontsize{8.5}{10}\selectfont ANONYMOUS MANUSCRIPT}
\fancyfoot[C]{\thepage}
...
\vspace{8pt}
{\headingfont\fontsize{11}{13}\selectfont Running title: DriftSense tangible task reflection\par}
\vspace{12pt}
{\headingfont\bfseries Anonymous Author\par}
{\headingfont Anonymous Institution\par}
\end{center}

\section*{Abstract}
\noindent Browser sites are mixed-use environments, so neither a hostname nor elapsed time can determine whether activity serves a user's current task. DriftSense is a local-first browser and tangible-interface system for participant-reported task drift. In Phase 1, 19 participants contributed 665 explicitly initiated task sessions containing structured task context, content-free activity counts, and post-session alignment reports. A compact logistic model showed strong completed-session discrimination on a chronological holdout (ROC--AUC = .938; F1 = .870). Phase 2 applied the model through successive five-minute windows for sessions up to 20 minutes and ten-minute windows for longer sessions. Windows above threshold activated an ESP32 interface with a countdown display, brief buzzer cue, and three physical controls. Interviews with 12 participants indicated high usability, privacy comfort, usefulness, and low annoyance, while identifying mistimed cues. The results support a user-defined, privacy-minimized approach to reflective assistance while leaving early-window accuracy and causal behavioral effects for future controlled evaluation.
...
\caption{Implemented DriftSense browser interfaces. The task-start popup makes the declared task and duration explicit, while the cropped controller view shows the browser-side LCD representation and state-dependent mapping of the three physical buttons.}
...
\node[text=dsPaleGreen,font=\ttfamily\bfseries\fontsize{9}{11}\selectfont,align=left] at (3.02,1.40) {TASK 18:42\\B3: FINISH};
...
\node[state,fill=dsPaleBlue] (select) at (8.95,1.35) {\headingfont\bfseries SET DURATION\\B1 reset \;|\; B2 +10 min};
\node[state,fill=dsPaleGreen] (running) at (12.65,1.35) {\headingfont\bfseries RUNNING\\LCD countdown; B3 finish};
\node[state,fill=dsPaleRed] (reflect) at (12.65,-1.25) {\headingfont\bfseries REFLECTION\\B1 aligned \;|\; B2 moved away\\B3 not sure};
\node[state,fill=dsPaleGold] (cue) at (16.25,-.20) {\headingfont\bfseries CHECK-IN\\threshold reached; two beeps\\every 10 s, up to five cycles};
\draw[arrow] (select.east)--node[above,edgelabel]{B3 start}(running.west);
\draw[arrow] (running.south)--node[left,edgelabel]{B3 finish}(reflect.north);
\draw[arrow] (running.south east)--(cue.north west);
\draw[arrow,draw=dsGreen] (cue.east) -- ++(.42,0) |- node[right,pos=.25,edgelabel]{return to task\\or alert stops}(running.east);
\node[rounded corners=2pt,draw=dsRule,fill=white,text width=9.2cm,align=center,inner sep=3pt,font=\fontsize{8.2}{9.5}\selectfont] at (12.35,-2.85) {The browser stores data and computes the score; the ESP32 displays state, sounds the cue, and returns button events.};
\end{tikzpicture}
}
\caption{Tangible interface and state-dependent controls. Panel (a) shows the physical LCD, buzzer, and three buttons. Panel (b) shows how the same controls support duration selection, a running task, a model-triggered check-in, and the final alignment response.}
\label{fig:hardware}
\end{figure}
...
\end{tikzpicture}
\caption{DriftSense privacy boundary. The local record contains only structured task context, aggregate behavior, and explicit participant responses. It excludes page semantics and identifiable content.}
\label{fig:privacy}
\end{figure}
...
\caption{Retrospective completed-session model results. Panel (a) shows how activity features separate the outcomes more strongly than context-only baselines. Panel (b) reports the separately packaged Phase 2 model at its stored threshold. These are completed-session results, not direct validation of five- or ten-minute windows.}
