...
\section*{Abstract}
\noindent Browser domains are mixed-use environments: the same site can support work or an unplanned detour, making hostname and elapsed time poor proxies for task alignment. DriftSense investigates \emph{browser task drift}---participant-reported deviation from a task declared at session start---through privacy-minimized local sensing and a tangible reflective interface. In Phase 1, 19 participants contributed 665 task sessions with structured context, content-free activity summaries, and post-session alignment reports. A compact logistic model achieved strong retrospective discrimination on a chronological known-participant holdout (ROC--AUC = .938; F1 = .870). In Phase 2, the frozen model evaluated five-minute windows for sessions up to 20 minutes and ten-minute windows for longer sessions. Scores above threshold activated a nonblocking ESP32 device with a countdown display, brief beep, and three controls. Interviews with 12 participants indicated high usability, privacy comfort, perceived usefulness, and low annoyance, while identifying mistimed cues. Early-window accuracy and causal behavioral effects remain for controlled evaluation.
...
At a checkpoint, a score below threshold produced no device cue. A score at or above threshold began one alert episode. The cue never displayed the score or stated that drift had occurred. The participant could return to the task, continue intentionally, or finish and answer the post-session question. The deployment delivered every cue whose score reached the threshold; it did not include a concurrent silent-control condition.
...
Automated verification covered the extension build, browser--Python probability agreement, duration-policy checkpoints, serial communication, export schema, outcome handling, and linked-record deletion. Browser and Python probabilities agreed for the serialized full-session test vectors to floating-point tolerance. Duration-policy checks confirmed minutes 5, 10, 15, and 20 for a 20-minute session and ten-minute steps for longer sessions; serial checks verified the three button events and the device states used for session control, reflection, and alerts.

Privacy checks confirmed that exports remain limited to documented fields, uncertain and action-only outcomes are not converted to aligned labels, page paths, query strings, and key values are excluded, and local deletion removes linked activity windows. These checks support RQ1 as an implemented system property, while participant comfort is reported separately.
...
\caption{Item-level favorable ratings from the post-study interviews. Fractions show favorable responses over valid responses; the bar length shows the same proportion.}
\label{fig:interview}
\end{figure}
...
This separation strengthens the contribution. The session data show that content-free session patterns carry useful retrospective signal; the deployed product shows that the decisions can drive a time-limited physical interaction; and the interviews identify both acceptance and concrete refinements. A future controlled study can build on these verified components without repeating the interviews.
