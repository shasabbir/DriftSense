...
.ds-dialog { width: min(680px, 100%); max-height: calc(100vh - 32px); overflow: auto; background: #fbfcfb; border: 1px solid rgba(255,255,255,.8); border-radius: 8px; box-shadow: 0 24px 70px rgba(12, 22, 18, .28); }
...
.ds-options { padding: 0 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.ds-option:last-child:nth-child(odd) { grid-column: 1 / -1; }
...
@media (max-width: 620px) { .ds-overlay { padding: 10px; } .ds-options, .ds-reflection-grid { grid-template-columns: 1fr; } .ds-option:last-child:nth-child(odd) { grid-column: auto; } .ds-actions { padding-top: 14px; } }
`
