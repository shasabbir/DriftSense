...
export interface CheckpointSnapshot { sessionId: string; anonymousUserId: string; cutoffSeconds: number; capturedAt: string; observable: boolean; clickCount: number; scrollCount: number; keyboardActivityCount: number; idleSeconds: number; activeSeconds: number; awaySeconds: number; tabSwitchCount: number; videoPlayingSeconds: number }
